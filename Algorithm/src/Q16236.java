...
//
//            bfs(sharkX, sharkY);
//            System.out.println(" 걸린시간 : "+count + " size = " +sharkSize +  " count = " + eatingCount);
...
//                            minX = newX;
//                            minY = newY;
//                            System.out.print((p.x-1) +" " + (p.y-1) + " -> " + (newX-1) + " " + (newY-1) +"잡아먹기");
	...
	public static void search() {
		while(!q.isEmpty()) {
			...
			for(int i = 0 ; i < 4; i++) {
				int nx = x + dx[i];
				int ny = y + dy[i];
				if(nx < 0 || ny < 0 || nx >= n || ny >= n || visited[nx][ny] || arr[nx][ny]>size) {		//이동못하는 곳
					continue;
				}
				times[nx][ny] = times[x][y] +1;															//시간증가
				visited[nx][ny] = true;
